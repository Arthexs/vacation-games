...
A game only implements what it needs beyond this baseline: `broadcastTvContent`/`clearTvContent` (+ a `public/games/<name>/tv.js`) for a shared-screen moment, `startTimer` (`src/timer.js`) for a countdown, `sendPlayerUpdate` for a private per-player update, `handleAdminAction(io, state, payload)` (+ a `public/games/<name>/admin.js`) for an admin-in-the-loop step like role assignment, `requestStop(io, state, finish)` for a graceful `admin:endGame`. None of these are required — most games use only `start`/`stop`/`handleAction`, same as `demoGame`.

`requestStop(io, state, finish)` — called by `adminSocket.js`'s `admin:endGame` instead of going straight to `stop()`, for a game where "someone answered this round, someone else hasn't" would otherwise mean the round gets silently discarded the instant the admin clicks End Game. The game decides when it's actually done and calls `finish()` itself — e.g. `higherLower` resolves its in-progress round right away (with whatever answers are already in, same as if the stragglers had just stayed connected-but-silent), lets players see the reveal, then calls `finish()` instead of starting another round. A game without `requestStop` ends immediately, exactly as before this existed.

A game's optional `public/games/<name>/admin.js` mirrors `play.js`'s `render(container, socket)` / `update(container, socket, payload)` shape, with one addition: both also receive a `helpers` object — `helpers.renderPlayerPicker(container, onPick)` renders the connected-player list as tappable buttons for role assignment (plus a "Select at Random" button that calls `onPick` with a random connected player, so the admin isn't forced to pick manually every round), and `helpers.getPlayers()` returns the current leaderboard array directly (e.g. to check there are enough connected players before offering role assignment at all) — so a game doesn't have to rebuild player-list rendering/tracking itself. Loaded by `public/js/admin.js` into `#gameControlSection`, shown alongside the "End Game" button while that game is active.
...
3. Real games — see `GAME_PLANS.md` for full specs and build order. `higherLower`, `spyfall`, `imposter`, `headsUp`, `wavelength`, `drawful` **done** (`witsAndWagers` was built and later removed); `psych` also done, built beyond `GAME_PLANS.md`'s original spec list. Built one at a time, each following `demoGame`'s shape, pulling in whichever of `GAME_PLANS.md`'s core contract additions it needs.
