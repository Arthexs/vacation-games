...
3. Real games, one at a time, each a new folder pair under `src/games/<name>/` and `public/games/<name>/`, following `demoGame`'s exact shape and registered in `src/games/index.js`. Don't touch core shell files to add a game — if a new game genuinely needs something the current contract doesn't support, stop and say so before changing shared code, per `GAME_PLANS.md`'s "Core contract additions" (§1–3, §5 built; §6 still open). `higherLower`, `spyfall`, `imposter`, `headsUp`, `wavelength`, `drawful` **done** (`witsAndWagers` was built and later removed — added little beyond the other games); `psych` also done, built beyond `GAME_PLANS.md`'s original spec list. Addition #6 (rules display) is the one core-contract piece from that doc still unbuilt. Check `GAME_PLANS.md` before assuming there's nothing left — it's a living doc the user edits directly.
